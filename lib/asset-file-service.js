...
import { buildAssetUnitStorageKey } from "@/lib/storage/storage-key";
import {
...
export async function saveAssetFileFromForm(formData, actor) {
  ...
  const storedName = `${assetUnit.assetCode}-${fileId}-${cleanName(path.basename(file.name, extension))}${extension}`;
  ...
  const storageKey = storage.name === "supabase"
    ? buildAssetUnitStorageKey({ assetUnitId, fileId, extension })
    : `${assetUnit.assetCode}/${storedName}`;
