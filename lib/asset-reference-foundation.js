...
export async function validateAssetItemFamily({ prismaClient, body, id = null }) {
  const current = id
    ? await prismaClient.assetItem.findFirst({ where: { id, deletedAt: null }, select: { categoryId: true, code: true } })
    : null;
  if (id && !current) fail("Référence matériel introuvable.", 404);
  if (body.name !== undefined && !value(body.name)) fail("Le nom de la référence est obligatoire.");
  ...
  const code = value(body.code ?? current?.code).toUpperCase();
  if (!code) fail("Le code de la référence est obligatoire.");
  const duplicate = await prismaClient.assetItem.findFirst({
    where: { code, deletedAt: null, ...(id ? { id: { not: id } } : {}) },
    select: { id: true }
  });
  if (duplicate) fail("Ce code de référence est déjà utilisé.", 409);
...
export function assertQuantitativeTrackingMode(assetItem, allowedModes = ["Q"]) {
  const family = assetItem?.category;
  if (!family || family.hierarchyLevel !== "FAMILY" || family.status !== "ACTIVE" || family.deletedAt) {
    fail("La référence quantitative doit être liée à une famille active.");
  }
  if (!allowedModes.includes(family.trackingMode)) {
    const error = new Error(
      `Le mode ${family.trackingMode || "I"} sera disponible après activation du flux correspondant. Aucune unité individuelle n'a été créée.`
    );
    error.code = "TRACKING_MODE_NOT_OPERATIONAL";
    error.status = 409;
    throw error;
  }
  return family.trackingMode;
}
