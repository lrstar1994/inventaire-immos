...
function preparedEntryData(body, actor, { quantity, supplierId, entryNumber }) {
  ...
  return {
    ...
    initialStatus: body.initialStatus, entryStatus: "VALIDATED", informationStatus: body.informationStatus || "PARTIAL",
...
export async function createQuantitativeAssetEntryWithPosition(body, actor, { prismaClient = prisma, createPosition } = {}) {
  ...
  return prismaClient.$transaction(async (tx) => {
    ...
    assertQuantitativeTrackingMode(assetItem);
    ...
    return { entry, quantitativePosition: position, units: [], duplicateWarning: null, trackingMode: "Q" };
  }, { maxWait: 10000, timeout: 30000 });
}
...
export async function createAssetEntryByTrackingMode(body, actor) {
  ...
  if (assetItem?.category?.trackingMode === "Q") return createQuantitativeAssetEntryWithPosition(body, actor);
