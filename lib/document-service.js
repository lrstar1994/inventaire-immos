...
export async function createDocumentFromEntries(body, actor) {
  ...
  try {
    ...
    const entries = await tx.assetEntry.findMany({
      ...
      include: {
        assetItem: { select: { id: true, name: true, code: true, category: { select: { trackingMode: true } } } },
    ...
    for (const entry of entries) {
      if (["Q", "QI"].includes(entry.assetItem?.category?.trackingMode)) {
        lines.push({
          ...
          lineLabel: `${entry.assetItem?.name || "Référence"} — quantité`,
...
export async function ensureEntryDraftDocument(tx, entryId, actor) {
  const existing = await tx.assetDocument.findFirst({
    where: {
      documentType: "ENTRY_SLIP",
      entries: { some: { assetEntryId: entryId } }
    },
    include: documentInclude(),
  ...
  const entry = await tx.assetEntry.findUnique({
    where: { id: entryId },
    include: {
      assetItem: { select: { id: true, name: true, code: true, category: { select: { trackingMode: true } } } },
      location: { select: { id: true, name: true, code: true } },
      ...
      assetUnits: {
        where: { deletedAt: null },
        include: { assetItem: { select: { name: true } } }
      }
    }
  });
  if (!entry) throw new Error("Entree introuvable pour la generation du bon d'entree.");
  if (entry.entryStatus !== "VALIDATED") throw new Error("Le bon d'entree automatique exige une entree validee.");

  const documentNumber = await generateDocumentNumber(tx, "ENTRY_SLIP", entry.entryDate);
  const documentId = `auto-entry-slip-${entry.id}`;
  const createdDocument = await tx.assetDocument.create({
    data: {
      id: documentId,
      documentNumber,
      ...
      title: `Bon d'entree - ${entry.entryNumber}`,
      status: "DRAFT",
      notes: entry.notes,
      createdById: actor?.id || null,
      updatedById: actor?.id || null,
      entries: { create: { assetEntryId: entry.id } }
    }
  });

  const trackingMode = entry.assetItem?.category?.trackingMode;
  const lines = ["Q", "QI"].includes(trackingMode)
    ? [{
        documentId: createdDocument.id,
        assetEntryId: entry.id,
        assetItemId: entry.assetItemId,
  ...
  if (lines.length) await tx.assetDocumentLine.createMany({ data: lines });

  await tx.auditLog.create({
    data: {
      action: "AUTOMATIC_ENTRY_SLIP_CREATED",
      entityTable: "asset_documents",
      entityId: createdDocument.id,
      summary: `Bon d'entree brouillon ${documentNumber} genere pour ${entry.entryNumber}`,
      metadata: JSON.stringify({ assetEntryId: entry.id, lineCount: lines.length }),
      userId: actor?.id || null
    }
  });
  return {
    document: await tx.assetDocument.findUnique({ where: { id: createdDocument.id }, include: documentInclude() }),
    created: true
  };
}
