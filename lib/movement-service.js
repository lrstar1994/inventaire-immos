...
export async function createMovement(body, actor) {
  ...
  const builtLines = await buildMovementLines(prisma, body.lines);
  ...
  if (relatedMovementId) {
    ...
    const related = await prisma.assetMovement.findUnique({
  ...
  const movementNumber = await generateMovementNumber(prisma, movementDate, builtLines.length);
  const transactionRequestedAt = Date.now();
  let transactionAcquiredAt = null;
  let transactionCompletedAt = null;
  const movement = await prisma.$transaction(async (tx) => {
    transactionAcquiredAt = Date.now();
  ...
  console.info(JSON.stringify({
    event: "asset_movement_transaction_completed",
    maxWaitMs: 10000,
    timeoutMs: 30000,
    preparationReads: relatedMovementId ? 4 : 3,
    transactionReads: 1,
    transactionWrites: 1,
    acquisitionMs: transactionAcquiredAt - transactionRequestedAt,
    transactionMs: transactionCompletedAt - transactionAcquiredAt,
    totalTransactionCallMs: Date.now() - transactionRequestedAt
  }));
  return movement;
}
...
export async function validateMovement(id, actor) {
  ...
  if (!normalizedId) throw new Error("Identifiant du mouvement obligatoire.");
  const transactionRequestedAt = Date.now();
  let transactionAcquiredAt = null;
  let transactionCompletedAt = null;
  let lineCount = 0;
  ...
  const movement = await prisma.$transaction(async (tx) => {
    transactionAcquiredAt = Date.now();
    await assertActiveDatabaseSchema(tx);
    const movement = await tx.assetMovement.findUnique({ where: { id: normalizedId }, include: movementInclude() });
    ...
    if (movement.movementStatus === "VALIDATED") {
    ...
    const validated = await tx.assetMovement.update({
    ...
    await tx.auditLog.createMany({
      data: [
        {
          action: "ASSET_MOVEMENT_VALIDATED",
          entityTable: "asset_movements",
          entityId: validated.id,
          summary: `ASSET_MOVEMENT_VALIDATED ${validated.movementNumber}`,
          metadata: JSON.stringify({ lineCount: validated.lines.length }),
          userId: actor.id
        },
        ...validated.lines.map((line) => ({
          ...
          summary: `Emplacement mis a jour par ${validated.movementNumber}`,
          metadata: JSON.stringify({
            movementId: validated.id,
            movementNumber: validated.movementNumber,
  ...
  console.info(JSON.stringify({
    event: "asset_movement_validation_transaction_completed",
    maxWaitMs: 10000,
    timeoutMs: 30000,
    lineCount,
    transactionReads: 1 + lineCount,
    transactionWrites: alreadyValidated ? 0 : lineCount + 2,
    acquisitionMs: transactionAcquiredAt - transactionRequestedAt,
    transactionMs: transactionCompletedAt - transactionAcquiredAt,
    totalTransactionCallMs: Date.now() - transactionRequestedAt
  }));
