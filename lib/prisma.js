...
import { assertQuantitativeFoundationSchema } from "@/lib/quantitative-foundation";
...
export async function assertActiveDatabaseSchema(client) {
  ...
  const schema = await assertExpectedSchema(client, expectedPostgreSQLSchema, databaseClientSelection);
  await assertQuantitativeFoundationSchema(client, schema);
  return schema;
}
