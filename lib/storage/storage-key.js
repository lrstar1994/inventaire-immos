...
export function buildAssetEntryStorageKey({ assetEntryId, fileId, extension }) {
  const safeAssetEntryId = assertSafeStorageSegment(assetEntryId, "assetEntryId");
  const safeFileId = assertSafeStorageSegment(fileId, "fileId");
  const safeExtension = normalizeFileExtension(extension);
  return `assets/entries/${safeAssetEntryId}/${safeFileId}/${safeFileId}.${safeExtension}`;
}
