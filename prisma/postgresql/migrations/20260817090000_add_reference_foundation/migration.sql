...
ALTER TABLE "immos"."asset_categories"
  ADD COLUMN "hierarchy_level" "immos"."AssetCategoryLevel" NOT NULL DEFAULT 'CATEGORY',
  ADD COLUMN "tracking_mode" "immos"."AssetTrackingMode",
  ADD COLUMN "control_level" "immos"."AssetControlLevel";

UPDATE "immos"."asset_categories"
SET "hierarchy_level" = 'SUBCATEGORY'
WHERE "parent_id" IN (
  SELECT "id" FROM "immos"."asset_categories" WHERE "parent_id" IS NULL
);
...
SET "hierarchy_level" = 'FAMILY',
    ...
    "control_level" = 'C1'
WHERE "parent_id" IN (
  SELECT "id" FROM "immos"."asset_categories" WHERE "hierarchy_level" = 'SUBCATEGORY'
);
...
BEGIN
  IF EXISTS (
    SELECT 1
    FROM "immos"."asset_categories" child
    LEFT JOIN "immos"."asset_categories" parent ON parent."id" = child."parent_id"
...
CREATE INDEX "asset_categories_hierarchy_level_idx"
  ON "immos"."asset_categories"("hierarchy_level");
